    ...
    <div class="card">
        <div class="card-header">
            <div class="d-flex justify-content-between">
                <h3 class="card-title text-wrap">{{ __('create_assistance_demand') }}</h3>
            </div>
        </div>
        <div class="card-body">
            <div class="row overflow-auto">
                <form method="POST" action="{{ route('assistance.store') }}" enctype="multipart/form-data">
                    @csrf
                    @method('POST')
                    <div class="row mb-3">
                    ...
                    <div class="row mb-3">
                        <div class="col-12 mt-3">
                            ...
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="checkbox" name="requestSubmitter[]"
                                       value="concernedPerson"
                                       id="concernedPerson" {{ old('requestSubmitter') && in_array('concernedPerson', old('requestSubmitter')) ? 'checked' : '' }}>
                                <label class="form-check-label"
                                       for="concernedPerson">{{ __('Concerned Person') }}</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="checkbox" name="requestSubmitter[]"
                                       value="otherPerson"
                                       id="otherPerson" {{ old('requestSubmitter') && in_array('otherPerson', old('requestSubmitter')) ? 'checked' : '' }}>
