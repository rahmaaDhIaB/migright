    ...
    <ul class="dropdown-menu  dropdown-menu-end  px- py-3 me-sm-n4  border" aria-labelledby="dropdownMenuButton">
        <li class="mb-2">
            <a class="dropdown-item border-radius-md"
               href="{{ request()->has('status') && request()->query('status') == 'pending' ? route('assistance.show', ['id' => $id, 'status' => 'pending']) : route('assistance.show', $id) }}">                    <span class="text-sm">{{__('show')}}</span>
                </a>
        </li>

        @if(auth()->user()->is_admin)
            <li class="mb-2">
                <a class="dropdown-item border-radius-md" href="{{route('assistance.edit',$id)}}">
            ...
            <li class="mb-2">
                <a class="dropdown-item border-radius-md" href="{{route('assistance.assignAssistanceDemandToUser.create',$id)}}">
                    <span class="text-sm">{{__('assign_to_user')}}</span>
                </a>
            </li>
        @endif
    </ul>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title text-wrap" id="exampleModalLabel">{{__('delete_assistance_modal')}}</h5>
            ...
            <div class="modal-footer">
                ...
                <form
                    method="post" action="{{route('assistance.destroy',$id)}}"
                >
                    @method('delete')
                    @csrf
