        ...
        <div class="card-header">
            ...
            <div class="card-body">
                <div class="row overflow-auto">
                    <form method="POST" action="{{ route('lostPerson.assignLostPersonDemandToUser.store',$id) }}"
                    >
                        @csrf
                        @method('POST')
                        <div class="row mb-3">
                            <div class="col-12 col-sm-12 mt-3 mt-sm-0">
                                ...
                                <select class="form-control" name="partner">
                                   @foreach($partners as $partner)
                                       <option value="{{$partner->id}}" {{$partner->id == $lostPersonDemand->demand->user_id ? 'selected' : ''}}>{{$partner->email}} </option>
                                   @endforeach
                                </select>
                            </div>
